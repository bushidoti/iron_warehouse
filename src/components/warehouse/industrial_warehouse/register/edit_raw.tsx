...
export const EditDocRaw = () => {
    const context = useContext(Context)
    const [loading, setLoading] = useState(true);
    const navigate = useNavigate();
    const [allProducts, setAllProducts] = useState<any>([])
    const [rawDetailed, setRawDetailed] = useState<any>([])
    const [form] = Form.useForm();
    const [listProduct, setListProduct] = useState<any[]>([]);

    const fetchData = async () => {
        await axios.get(
            `${Url}/api/${context.currentProductDoc === 'factor' ?  'raw_material_factor' :  'raw_material_check' }/${context.currentProductDoc === 'factor' ?  context.currentProductFactor :  context.currentProductCheck }/`, {
                headers: {
            ...
            return await axios.get(`${Url}/api/raw_material_detailed/?systemID=${context.currentProductDoc === 'factor' ?  context.currentProductFactor :  context.currentProductCheck }`, {
                headers: {
                    'Authorization': 'Bearer ' + localStorage.getItem('access_token'),
                }
            })
            ...
            return await axios.get(`${Url}/api/raw_material_detailed`, {
                headers: {
                    'Authorization': 'Bearer ' + localStorage.getItem('access_token'),
                }
            })
            ...
            setRawDetailed(data.data)
        }).then(async () => {
            return await axios.get(`${Url}/api/raw_material`, {
                headers: {
                    'Authorization': 'Bearer ' + localStorage.getItem('access_token'),
                }
            })
    ...
    const onFinish = async () => {
        if (context.currentProductDoc === 'check'){
            ...
            form.getFieldValue(['products']).map((obj:
              ...
              form.getFieldValue(['products']).map(async (product: { product: number, systemID: number; }, i: number) => {
                form.setFieldsValue({
                    products: {
                        [i]: {
                            afterOperator: (rawDetailed.filter((products: {
                                    product: number;
                                }) => products.product === product.product).reduce((a: any, v: {
                                    input: any;
                                }) => a + v.input, 0))
                                - (rawDetailed.filter((products: {
                                    product: number;
                                }) => products.product === product.product).reduce((a: any, v: {
                                    output: any;
                                }) => a + v.output, 0)) - (rawDetailed.filter((products: {
                                    systemID: number;
                                    product: number;
                            ...
                            average_rate: context.department !== 'مدیریت مالی' ? undefined : rawDetailed.filter((products: {
                                    product: number;
                                }) => products.product === product.product).length === 0 ? form.getFieldValue(['products'])[i].rate : ((rawDetailed.filter((products: {
                                    product: number;
            ...
            async () => {
                await axios.put(
                    `${Url}/api/raw_material_check/${form.getFieldValue(['CheckID'])}/`, {
                    ...
                    async () => {
                        allProducts.map(async (data: { id: any; } , i: number) => (
                            await axios.put(`${Url}/api/raw_material_detailed/${data.id}/`,form.getFieldValue(['products'])[i], {
                                headers: {
                                    'Authorization': 'Bearer ' + localStorage.getItem('access_token'),
                                }
                            }).then(
            ...
            async () => {
                await axios.put(
                    `${Url}/api/raw_material_factor/${form.getFieldValue(['FactorID'])}/`, {
                    ...
                    async () => {
                        form.getFieldValue(['products']).map(async (data: { product: any, systemID: number; } , i: number) => (
                            await axios.put(`${Url}/api/raw_material/${data.product}/`, {
                            left: (rawDetailed.filter((products: {
                                    product: number;
                            ...
                            average_rate: context.department !== 'مدیریت مالی' ? undefined :  rawDetailed.filter((products: {
                                    product: number;
                                }) => products.product === data.product).slice(-2)[0].length === 0 ? form.getFieldValue(['products'])[i].rate : ((rawDetailed.filter((products: {
                                    product: number;
