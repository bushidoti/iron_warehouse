...
import qs from "qs";
import TablePrint from "../ptrint_table/table_print_industrial";
...
const IndustrialTable: React.FC = () => {
    ...
    const fetchData = async () => {
       setLoading(true)
        await axios.get(`${Url}/api/property/?size=${pagination.pageSize}&page=${pagination.current}&fields=code,movement_status,category,factorCode,name,property_number,document_code,model,year_buy,user,using_location,movement_message&${qs.stringify(filteredInfo, {
                encode: false,
                arrayFormat: 'comma'
    ...
    const columns: ColumnsType<DataType> = [
        {
            ...
            render: (_value, record) => <Button disabled={record.movement_status === 'ارسال شده'} type={"link"} onClick={() => {
                ...
                navigate(`/property/industrial/edit/${record.code}`)
             }}>{record.code}</Button>,
        }, {
            align: "center",
            title: 'نام اموال',
