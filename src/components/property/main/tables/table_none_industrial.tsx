...
import qs from "qs";
import TablePrint from "../ptrint_table/table_print_none_industrial";
...
const NoneIndustrialTable: React.FC = () => {
    ...
    const fetchData = async () => {
       setLoading(true)
        await axios.get(`${Url}/api/property/?size=${pagination.pageSize}&page=${pagination.current}&fields=code,movement_status,category,factorCode,inventory,name,property_number,document_code,year_buy,user,using_location,movement_message&${qs.stringify(filteredInfo, {
                encode: false,
                arrayFormat: 'comma'
    ...
    const columns: ColumnsType<DataType> = [
        {
            ...
            render: (_value, record) => <Button disabled={record.movement_status === 'ارسال شده'} type={"link"} onClick={() => {
                ...
                navigate(`/warhouse/property/none_industrial/edit/${record.code}`)
             }}>{record.code}</Button>,
        }, {
            align: "center",
            title: 'نام اموال',
...
export default NoneIndustrialTable;